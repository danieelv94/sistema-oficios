<x-app-layout>
    ...
    <div class="py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            {{-- ========================================================== --}}
            <div x-data="pushSubscription()" 
                 x-init="checkSubscription()" 
                 x-show="!isSubscribed && showBanner" 
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h4 class="text-lg font-black uppercase tracking-tighter italic">¿Quieres recibir alertas de soporte?</h4>
                            <p class="text-xs opacity-90 font-medium uppercase tracking-widest leading-relaxed">
                                Activa las notificaciones para saber al instante cuando tu ticket sea resuelto o existan nuevos avisos.
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-t-4 border-[#932C43]">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center mb-6">
                        ...
                        <h3 class="text-xl font-extrabold text-gray-700 uppercase tracking-tight">Solicitudes en Proceso</h3>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($ticketsPendientes as $ticket)
                                    <tr class="hover:bg-red-50 transition-colors">
                                        ...
                                        @if(Auth::user()->role == 'admin')
                                            <td class="py-3 px-4">
                                                <div class="text-sm font-bold text-gray-800">{{ $ticket->user ? $ticket->user->name : 'Usuario Inactivo' }}</div>
                                                <div class="text-[10px] text-gray-500 uppercase">{{ ($ticket->user && $ticket->user->area) ? $ticket->user->area->name : 'Sin Área' }}</div>
                                            </td>
                                        @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-10 text-gray-400 italic font-medium">No hay folios pendientes de atención.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg border-t-4 border-gray-400">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center mb-6">
                        ...
                        <h3 class="text-xl font-bold text-gray-500 uppercase tracking-tight">Historial de Soluciones</h3>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($ticketsConcluidos as $ticket)
                                    <tr class="hover:bg-gray-50 grayscale-[0.5] transition-all">
                                        <td class="py-3 px-4 text-sm text-gray-500">{{ $ticket->completed_at ? \Carbon\Carbon::parse($ticket->completed_at)->format('d/m/Y H:i') : 'N/A' }}</td>
                                        @if(Auth::user()->role == 'admin')
                                            <td class="py-3 px-4 text-sm">{{ $ticket->user ? $ticket->user->name : 'Ex-empleado' }}</td>
                                        @endif
                                        <td class="py-3 px-4 text-sm text-gray-400 italic">{{ $ticket->subject }}</td>
                                        <td class="py-3 px-4 text-center">
                                            <a href="{{ route('tickets.show', $ticket) }}" class="inline-block px-4 py-1 bg-gray-400 text-white rounded text-xs font-bold hover:bg-gray-500">Revisar</a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-8 text-gray-300 italic text-sm">El historial está vacío.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    {{-- ========================================================== --}}
    {{-- SCRIPTS PARA WEB PUSH Y SUSCRIPCIÓN                        --}}
    {{-- ========================================================== --}}
    <script>

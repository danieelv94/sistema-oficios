...
<body class="bg-gray-100 p-8">
    ...
    <div class="max-w-4xl mx-auto mb-4 text-right no-print">
        <a href="{{ route('comisiones.index') }}"
            class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 font-sans">&larr; Volver al Listado</a>
        @if($comision->status !== 'Cancelado' || Auth::user()->role == 'admin')
            <button onclick="window.print()"
                class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-sans">Imprimir</button>
        @endif
    </div>
    ...
    <div class="max-w-4xl mx-auto bg-white p-12 printable-area relative">
        @if($comision->status === 'Cancelado')
            <div class="absolute inset-0 flex items-center justify-center z-0">
                <p style="font-size: 12rem; line-height: 1;"
                    class="font-black text-red-500 opacity-20 transform -rotate-45 select-none">CANCELADO</p>
            </div>
        @endif

        <div class="relative z-10">
            <div class="flex justify-end mb-8">
                <img src="{{ asset('images/encabezado.png') }}" alt="Encabezado del Oficio"
                    style="width: 9.09cm; height: 2.39cm;">
            </div>

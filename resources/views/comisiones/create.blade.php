                    ...
                    <form action="{{ route('comisiones.store') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="border-t pt-6">
                                ...
                                <select name="vehiculo_id" id="vehiculo_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                    <option value="">No se requiere vehículo</option>
                                    @foreach($vehiculos as $vehiculo)
                                        <option value="{{ $vehiculo->id }}">{{ $vehiculo->marca }} {{ $vehiculo->modelo }} - Placas: {{ $vehiculo->placa }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="border-t pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="proyecto_id" id="proyecto_id" x-model="selectedProyecto" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                ...
                                <div x-show="unidades.length > 0" x-transition>
                                    <label for="unidad_administrativa_id" class="block font-medium text-sm text-gray-700">Unidad Administrativa</label>
                                    <select name="unidad_administrativa_id" id="unidad_administrativa_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" :disabled="!selectedProyecto">
                                        <option value="">Selecciona una unidad...</option>
                                        <template x-for="unidad in unidades" :key="unidad.id">
                                            <option :value="unidad.id" x-text="unidad.clave + ' ' + unidad.nombre"></option>
                                        </template>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-end mt-8 border-t pt-6">
                            ...
                            <x-button class="ml-4">Generar Oficio</x-button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            return {
                ...
                init() {
                    this.$watch('selectedProyecto', (newVal) => {
                        document.getElementById('unidad_administrativa_id').value = ''; // Resetea la selección
